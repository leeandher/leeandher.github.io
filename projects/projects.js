var urls = [
  ...
  'https://goo.gl/J74Vum',
  'https://goo.gl/fgrRKB',
  'https://images.pexels.com/photos/209137/pexels-photo-209137.jpeg',
...
function applyProject(proj, order) {
  ...
  document.querySelector(projId + ' .proj-title').textContent = proj.title;
  document.querySelector(projId + ' .live').setAttribute('href','https://leeandher.github.io/projects/' +  proj.link)//'subdir/' + linkUrl;
  document.querySelector(projId + ' .source').setAttribute('href', 'https://github.com/leeandher/leeandher.github.io/tree/master/projects/' + proj.link)//'subdir/' + linkUrl;
  document.querySelector(projId + ' .proj-date').textContent = proj.date;
  document.querySelector(projId + ' .proj-desc').textContent = proj.desc;
  document.querySelector(projId + ' .languages').textContent = arrToList(proj.languages);
  document.querySelector(projId + ' .additional').textContent = arrToList(proj.additional);
  document.querySelector(projId + ' .concepts').textContent = arrToList(proj.concepts);
  document.querySelector(projId + ' .proj-bg').style.backgroundImage = proj.bg;
}
